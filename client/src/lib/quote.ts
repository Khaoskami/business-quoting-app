...
export function buildPrintHtml(q: any, biz: any) {
  ...
  const mc = (v: number) => `<span class="money" data-base="${Number(v) || 0}">${money(v, cur)}</span>`;
  const itemRows = q.items.map((i: any) =>
    `<tr><td style="padding:8px;border-bottom:1px solid #e5e5e5">${esc(i.description)}</td><td style="padding:8px;border-bottom:1px solid #e5e5e5;text-align:center">${i.quantity}</td><td style="padding:8px;border-bottom:1px solid #e5e5e5;text-align:right">${mc(i.unitPrice)}</td><td style="padding:8px;border-bottom:1px solid #e5e5e5;text-align:right">${mc(i.quantity * i.unitPrice)}</td></tr>`
  ).join('');
