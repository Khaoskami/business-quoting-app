...
const AUTH = {
  PREFIX: "bq_",
  SALT_KEY: "bq_auth_salt",
  HASH_KEY: "bq_auth_hash",
  LOCKOUT_KEY: "bq_auth_lockout",
  ...
  const save = useCallback(async (next) => {
    ...
    try {
      ...
      if (e?.name === "QuotaExceededError" || e?.code === 22) {
        window.dispatchEvent(new CustomEvent("bq:storage-full"));
      }
    }
  }, [fk, cryptoKey]);
...
function AuthenticatedApp({ cryptoKey, onLock, onKeyChange }) {
  ...
  const [tierData, setTierData, q5] = useStore("tier", { plan: "free" }, cryptoKey);
  const [licenseData, setLicenseData, q6] = useStore("license_key", { key: "" }, cryptoKey);
  ...
  // Re-verify license key on load — fall back to free if invalid
  useEffect(() => {
    if (!q6) return;
    if (!licenseData.key) {
      if (tier !== "free" && q5) setTierData({ plan: "free" });
      return;
    }
    const verified = verifyLicenseKey(licenseData.key);
    if (!verified) {
      setLicenseData({ key: "" });
      if (tier !== "free") setTierData({ plan: "free" });
    } else if (verified !== tier) {
      setTierData({ plan: verified });
    }
  }, [q5, q6, licenseData.key]);

  // Storage-full toast
  useEffect(() => {
    function handler() { notify("Storage full — export your data or delete old quotes.", "error"); }
    window.addEventListener("bq:storage-full", handler);
    return () => window.removeEventListener("bq:storage-full", handler);
  }, []);
