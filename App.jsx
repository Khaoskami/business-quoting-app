...
/* ==========================================================================
   BUSINESS QUOTING APPLICATION — PRODUCTION BUILD
   
   Copyright (c) 2026 Khaos / Khaoskami. All rights reserved.
   Unauthorized reproduction, modification, or distribution prohibited.
   Cryptographic integrity signatures embedded in every export.
...
const ENC = {
  ALG: "AES-GCM", LEN: 256, IV: 12, SK: "bq_sk", PFX: "bq_",
  async key() {
    const s = sessionStorage.getItem(this.SK);
    if (s) return crypto.subtle.importKey("raw", Uint8Array.from(atob(s), c => c.charCodeAt(0)), this.ALG, true, ["encrypt", "decrypt"]);
    const k = await crypto.subtle.generateKey({ name: this.ALG, length: this.LEN }, true, ["encrypt", "decrypt"]);
    sessionStorage.setItem(this.SK, btoa(String.fromCharCode(...new Uint8Array(await crypto.subtle.exportKey("raw", k)))));
    return k;
  },
  async enc(d) { const k = await this.key(), iv = crypto.getRandomValues(new Uint8Array(this.IV)), c = await crypto.subtle.encrypt({ name: this.ALG, iv }, k, new TextEncoder().encode(JSON.stringify(d))); return JSON.stringify({ iv: btoa(String.fromCharCode(...iv)), d: btoa(String.fromCharCode(...new Uint8Array(c))) }); },
  async dec(s) { try { const k = await this.key(), { iv, d } = JSON.parse(s); return JSON.parse(new TextDecoder().decode(await crypto.subtle.decrypt({ name: this.ALG, iv: Uint8Array.from(atob(iv), c => c.charCodeAt(0)) }, k, Uint8Array.from(atob(d), c => c.charCodeAt(0))))); } catch { return null; } },
};
...
async function signQuote(quote, owner) {
  const payload = JSON.stringify({ id: quote.id, title: quote.title, items: quote.items, total: calcTotals(quote.items, quote.taxPercent, quote.discountPercent).total, created: quote.createdAt, owner: owner.name || "Business Quotes App", ownerContact: owner.email || "", copyright: `© ${new Date().getFullYear()} ${owner.name || "Khaoskami"}. All rights reserved.`, timestamp: new Date().toISOString() });
...
function useStore(key, fb) {
  ...
  const fk = ENC.PFX + key;
  useEffect(() => { let s = false; (async () => { try { const r = localStorage.getItem(fk); if (r) { const d = await ENC.dec(r); if (!s && d !== null) setData(d); } } catch {} if (!s) setOk(true); })(); return () => { s = true; }; }, [fk]);
  const save = useCallback(async (n) => { const c = san(n); setData(c); try { localStorage.setItem(fk, await ENC.enc(c)); } catch {} }, [fk]);
  return [data, save, ok];
}
...
function newQuote(cur) {
  return { id: uid(), quoteNumber: "", title: "", clientId: "", clientName: "", clientUrl: "", status: "draft", currency: cur || "ZAR", taxPercent: 15, discountPercent: 0, validityDays: 30, notes: "", createdAt: new Date().toISOString(), items: [{ id: uid(), description: "", quantity: 1, unitPrice: 0, catalogId: "" }], signature: "", signedAt: "" };
}
...
const X = { bg: "#0e0e11", sf: "#16161a", bd: "#25252b", hv: "#1c1c22", tx: "#d4d4d8", mt: "#6b6b76", pr: "#3b82f6", ok: "#22c55e", no: "#ef4444", wn: "#f59e0b", ac: "#8b5cf6" };
...
const $ = {
  ...
  pro: { background: `${X.ac}12`, border: `1px solid ${X.ac}30`, borderRadius: 8, padding: "12px 16px", marginBottom: 12 },
  proBtn: { background: X.ac, color: "#fff", border: "none", borderRadius: 6, padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" },
};

// ---------------------------------------------------------------------------
// PAYMENT LINK — REPLACE THIS WITH YOUR REAL STRIPE/PAYFAST URL
// ---------------------------------------------------------------------------
...
function UpgradeBlock({ label }) {
  return <div style={$.pro}><div style={{ fontSize: 13, color: X.ac, fontWeight: 600, marginBottom: 6 }}>Pro Feature</div><div style={{ fontSize: 12, color: X.mt, marginBottom: 10 }}>{label || "Upgrade to Pro to unlock."}</div><button onClick={() => window.open(PAYMENT_URL, "_blank")} style={$.proBtn}>Upgrade — {TIERS.pro.price}</button></div>;
}

function UsageMeter({ current, max, label }) {
  const pct = Math.min(100, max === Infinity ? 0 : (current / max) * 100);
  const over = max !== Infinity && current >= max;
  return <div style={{ marginBottom: 8 }}><div style={{ ...$.btwn, marginBottom: 4 }}><span style={{ fontSize: 12, color: X.mt }}>{label}</span><span style={{ fontSize: 12, color: over ? X.no : X.tx, fontWeight: 600 }}>{current} / {max === Infinity ? "∞" : max}</span></div><div style={{ height: 4, background: X.bd, borderRadius: 2, overflow: "hidden" }}><div style={{ height: "100%", width: max === Infinity ? "0%" : `${pct}%`, background: over ? X.no : pct > 80 ? X.wn : X.pr, borderRadius: 2, transition: "width 0.3s" }} /></div></div>;
}

// ---------------------------------------------------------------------------
// PRINT + CSV HELPERS (shared between pages)
// ---------------------------------------------------------------------------
...
function buildPrintHtml(q, biz) {
  ...
  const sigBlock = q.signature ? `<div style="margin-top:24px;padding:12px;background:#f8f8f8;border:1px solid #ddd;border-radius:4px;font-size:11px;color:#666"><strong>INTEGRITY SIGNATURE</strong><br/><code style="font-size:10px;word-break:break-all">${q.signature}</code><br/>Signed: ${q.signedAt}<br/>Any modification invalidates this hash.</div>` : "";
  const copyright = `<div style="margin-top:32px;padding-top:12px;border-top:1px solid #ddd;font-size:10px;color:#999;text-align:center">\u00A9 ${new Date().getFullYear()} ${biz.name || "Khaoskami"} \u00B7 Generated by Business Quotes App \u00B7 github.com/Khaoskami<br/>Unauthorized reproduction prohibited.</div>`;
...
function Dash({ quotes, currency, tier, onNew, onEdit, search, setSearch }) {
  ...
  const stats = [
    { l: "This Month", v: `${mu}/${mMax === Infinity ? "∞" : mMax}`, c: atLimit ? X.no : X.pr },
    { l: "Pending", v: quotes.filter(q => q.status === "sent").length, c: X.wn },
    { l: "Accepted", v: accepted.length, c: X.ok },
    { l: "Revenue", v: money(revenue, currency), c: X.ac },
    { l: "Win Rate", v: winRate + "%", c: "#06b6d4" },
  ];

  return (
    <div>
      <div style={{ ...$.btwn, marginBottom: 20, flexWrap: "wrap" }}>
        <h2 style={$.h2}>Quotes</h2>
        <div style={$.row}>
          <span style={$.badge(tier === "free" ? X.mt : X.ac)}>{TIERS[tier].name}</span>
          <button onClick={onNew} disabled={atLimit} style={{ ...$.btn, ...(atLimit ? { opacity: 0.4, cursor: "not-allowed" } : {}) }}>+ New Quote</button>
        </div>
      </div>

      {atLimit && <div style={$.pro}><div style={{ fontSize: 14, fontWeight: 600, color: X.ac, marginBottom: 4 }}>Monthly limit reached</div><div style={{ fontSize: 12, color: X.mt, marginBottom: 10 }}>You have used all {mMax} quotes this month.</div><button style={$.proBtn} onClick={() => window.open(PAYMENT_URL, "_blank")}>Upgrade to Pro — {TIERS.pro.price}</button></div>}

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 8, marginBottom: 20 }}>
        {stats.map((s, i) => <div key={i} style={$.stat(s.c)}><div style={{ fontSize: 11, color: X.mt, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 4 }}>{s.l}</div><div style={{ fontSize: 17, fontWeight: 700, color: s.c }}>{s.v}</div></div>)}
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ ...$.grid, marginBottom: 16 }}>
        ...
        {tf.discount ? <Field label="Discount %" type="number" value={q.discountPercent} onChange={v => set("discountPercent", Math.min(100, Math.max(0, Number(v) || 0)))} min={0} max={100} /> : <div><label style={$.lbl}>Discount %</label><div style={{ fontSize: 12, color: X.ac, padding: "8px 0" }}>Pro feature</div></div>}
        <Field label="Valid (days)" type="number" value={q.validityDays} onChange={v => set("validityDays", Math.max(1, Number(v) || 1))} min={1} />
        {tf.clientUrl ? <Field label="Client Website" value={q.clientUrl} onChange={v => set("clientUrl", v)} placeholder="https://clientsite.com" span /> : <div style={{ gridColumn: "1 / -1" }}><label style={$.lbl}>Client Website</label><div style={{ fontSize: 12, color: X.ac, padding: "8px 0" }}>Pro feature</div></div>}
      </div>
      ...
      <div style={$.card}>
        ...
        <div style={$.divider} />
        <div style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", fontSize: 18, fontWeight: 700, color: X.ok }}><span>Total</span><span>{money(total, q.currency)}</span></div>
      </div>

      {/* SIGNATURE */}
      {q.signature ? <div style={{ background: `${X.ok}08`, border: `1px solid ${X.ok}25`, borderRadius: 8, padding: 12, marginBottom: 12 }}><div style={{ fontSize: 12, fontWeight: 600, color: X.ok, marginBottom: 4 }}>Signed ✓</div><div style={{ fontSize: 10, color: X.mt, fontFamily: "monospace", wordBreak: "break-all" }}>{q.signature}</div><div style={{ fontSize: 10, color: X.mt, marginTop: 4 }}>Signed: {q.signedAt}</div></div> : tf.signature ? <button onClick={async () => { const sig = await signQuote(q, biz); setQ(p => ({ ...p, signature: sig, signedAt: new Date().toISOString() })); notify("Signed."); }} style={{ ...$.btn2, marginBottom: 12, width: "100%" }}>Sign Quote (SHA-256)</button> : <div style={{ ...$.pro, marginBottom: 12 }}><div style={{ fontSize: 12, color: X.ac }}>Cryptographic signing is a Pro feature.</div></div>}
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        {tf.print ? <button onClick={() => { const w = window.open("", "_blank"); if (w) { w.document.write(buildPrintHtml(q, biz)); w.document.close(); w.print(); } }} style={$.btn2}>Print / PDF</button> : <button disabled style={{ ...$.btn2, opacity: 0.4, cursor: "not-allowed" }}>Print (Pro)</button>}
        <button onClick={() => { const csv = buildCsvRows(q, biz).map(r => r.map(c => `"${c}"`).join(",")).join("\n"); const b = new Blob([csv], { type: "text/csv" }); const u = URL.createObjectURL(b); Object.assign(document.createElement("a"), { href: u, download: `${q.quoteNumber || q.title || "quote"}.csv` }).click(); URL.revokeObjectURL(u); notify("CSV exported."); }} style={$.btn2}>CSV</button>
        {tf.duplicate && <button onClick={() => { setQ({ ...q, id: uid(), title: q.title + " (copy)", quoteNumber: "", status: "draft", createdAt: new Date().toISOString(), signature: "", signedAt: "" }); notify("Duplicated."); }} style={$.btnG}>Duplicate</button>}
...
function Clients({ clients, tier, onSave, onDelete, notify }) {
  ...
  return (
    <div>
      <div style={{ ...$.btwn, marginBottom: 20 }}><h2 style={$.h2}>Clients</h2><button onClick={() => { if (maxC !== Infinity && clients.length >= maxC) { notify("Limit reached. Upgrade.", "error"); return; } setForm(newClient()); }} style={$.btn}>+ Add</button></div>
      <UsageMeter current={clients.length} max={maxC} label="Clients" />
      {form && <div style={$.card}><div style={$.grid}>{fields.map(f => { if (f.key === "website" && !TIERS[tier].features.clientUrl) return <div key={f.key}><label style={$.lbl}>Website</label><div style={{ fontSize: 12, color: X.ac, padding: "8px 0" }}>Pro feature</div></div>; return <Field key={f.key} label={f.l} value={form[f.key]} onChange={v => setForm(p => ({ ...p, [f.key]: v }))} placeholder={f.ph} span={f.span} />; })}</div><div style={{ ...$.row, marginTop: 14 }}><button onClick={() => { if (!form.name.trim()) { notify("Name required.", "error"); return; } onSave({ ...form, id: form.id || uid(), website: validateUrl(form.website) }); setForm(null); }} style={$.btn}>Save</button><button onClick={() => setForm(null)} style={$.btnG}>Cancel</button></div></div>}
...
function Catalog({ catalog, tier, onSave, onDelete, notify }) {
  ...
  return (
    <div>
      <div style={{ ...$.btwn, marginBottom: 20 }}><h2 style={$.h2}>Products & Services</h2><button onClick={() => { if (maxP !== Infinity && catalog.length >= maxP) { notify("Limit reached. Upgrade.", "error"); return; } setForm(newProduct()); }} style={$.btn}>+ Add</button></div>
...
function Settings({ biz, tier, setTier, onSave, notify }) {
  const [form, setForm] = useState({ ...biz });
  return (
    <div>
      <h2 style={$.h2}>Settings</h2>
      <div style={{ ...$.card, marginBottom: 20 }}><div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>Your Plan</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 10 }}>{Object.entries(TIERS).map(([k, t]) => <div key={k} onClick={() => { if (k !== "free") window.open(PAYMENT_URL, "_blank"); setTier(k); }} style={{ border: `2px solid ${tier === k ? X.pr : X.bd}`, borderRadius: 8, padding: 14, cursor: "pointer", background: tier === k ? `${X.pr}08` : "transparent" }}><div style={{ fontWeight: 700, fontSize: 15, color: tier === k ? X.pr : X.tx, marginBottom: 4 }}>{t.name}</div><div style={{ fontSize: 13, color: X.mt, marginBottom: 8 }}>{t.price || "Free forever"}</div><div style={{ fontSize: 12, color: X.mt, lineHeight: 1.6 }}>{t.quotesPerMonth === Infinity ? "∞" : t.quotesPerMonth} quotes/mo<br/>{t.maxClients === Infinity ? "∞" : t.maxClients} clients<br/>{t.features.print ? "✓" : "—"} Print/PDF<br/>{t.features.discount ? "✓" : "—"} Discounts<br/>{t.features.signature ? "✓" : "—"} Signatures<br/>{t.features.clientUrl ? "✓" : "—"} Client URLs<br/>{t.features.duplicate ? "✓" : "—"} Duplicate</div></div>)}</div></div>
      <div style={$.card}><div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>Business Info</div><div style={$.grid}><Field label="Business Name" value={form.name || ""} onChange={v => setForm(p => ({ ...p, name: v }))} placeholder="Your Business" /><Field label="Email" value={form.email || ""} onChange={v => setForm(p => ({ ...p, email: v }))} placeholder="info@business.co" /><Field label="Phone" value={form.phone || ""} onChange={v => setForm(p => ({ ...p, phone: v }))} placeholder="+27 00 000 0000" /><Field label="Tax / VAT" value={form.taxId || ""} onChange={v => setForm(p => ({ ...p, taxId: v }))} placeholder="VAT4830000000" /><Field label="Website" value={form.website || ""} onChange={v => setForm(p => ({ ...p, website: v }))} placeholder="https://yourbusiness.co" /><Sel label="Currency" value={form.defaultCurrency || "ZAR"} onChange={v => setForm(p => ({ ...p, defaultCurrency: v }))} options={CURRENCIES.map(c => ({ value: c.code, label: `${c.code} — ${c.symbol}` }))} /><Field label="Address" value={form.address || ""} onChange={v => setForm(p => ({ ...p, address: v }))} placeholder="123 Main Rd" span /><div style={{ gridColumn: "1 / -1" }}><label style={$.lbl}>Default Terms</label><textarea value={form.terms || ""} onChange={e => setForm(p => ({ ...p, terms: e.target.value }))} rows={3} placeholder="Payment due within 30 days..." style={$.ta} /></div></div><button onClick={() => { onSave(form); notify("Saved."); }} style={{ ...$.btn, marginTop: 14 }}>Save</button></div>
      <h3 style={{ fontSize: 15, fontWeight: 700, margin: "24px 0 10px" }}>Security</h3>
      {[{ t: "AES-256-GCM Encryption", d: "All data encrypted with unique IVs." }, { t: "Session-Scoped Keys", d: "Key destroyed on tab close." }, { t: "Zero Network", d: "No data leaves your device." }, { t: "XSS + URL Sanitisation", d: "All inputs entity-encoded. URLs validated." }, { t: "SHA-256 Signatures", d: "Tamper-proof integrity hashes." }, { t: "IP Watermarking", d: "Copyright in every export." }].map((s, i) => <div key={i} style={{ background: X.bg, border: `1px solid ${X.bd}`, borderRadius: 6, padding: "10px 14px", marginBottom: 6 }}><div style={{ fontSize: 13, fontWeight: 600, color: X.ok, marginBottom: 2 }}>{s.t}</div><div style={{ fontSize: 12, color: X.mt }}>{s.d}</div></div>)}
      <div style={{ marginTop: 20, padding: 14, background: X.sf, border: `1px solid ${X.bd}`, borderRadius: 8 }}><div style={{ fontSize: 12, color: X.no, fontWeight: 600, marginBottom: 6 }}>DANGER ZONE</div><p style={{ fontSize: 12, color: X.mt, margin: "0 0 10px" }}>Permanently delete all data.</p><ConfirmBtn label="Delete All Data" confirmLabel="Confirm — delete everything?" onConfirm={() => { Object.keys(localStorage).filter(k => k.startsWith(ENC.PFX)).forEach(k => localStorage.removeItem(k)); window.location.reload(); }} style={$.btnD} /></div>
    </div>
  );
}
...
export default function App() {
  const [quotes, setQuotes, q1] = useStore("quotes", []);
  const [clients, setClients, q2] = useStore("clients", []);
  const [catalog, setCatalog, q3] = useStore("catalog", []);
  const [biz, setBiz, q4] = useStore("biz", { name: "", email: "", phone: "", address: "", taxId: "", website: "", defaultCurrency: "ZAR", terms: "" });
  const [tierData, setTierData, q5] = useStore("tier", { plan: "free" });
  ...
  const tier = tierData.plan || "free";
  const setTier = (p) => setTierData({ plan: p });
  const ready = q1 && q2 && q3 && q4 && q5;
  ...
  if (!ready) return <div style={{ ...$.root, alignItems: "center", justifyContent: "center" }}><span style={{ color: X.mt }}>Loading...</span></div>;
  ...
  return (
    <div style={$.root}>
      ...
      <header style={$.hdr}><h1 style={{ fontSize: 16, fontWeight: 700, margin: 0 }}>{biz.name || "Business Quotes"}</h1><div style={$.row}><span style={$.badge(tier === "free" ? X.mt : X.ac)}>{TIERS[tier].name}</span><span style={{ fontSize: 11, color: X.ok, fontWeight: 600, background: X.ok + "15", padding: "3px 8px", borderRadius: 4 }}>Encrypted</span></div></header>
      <nav style={$.nav}>{tabs.map(t => <Tab key={t.id} label={t.l} active={page === t.id || (page === "editor" && t.id === "dashboard")} onClick={() => go(t.id)} />)}</nav>
      <main style={$.main}>
        {page === "dashboard" && <Dash quotes={quotes} currency={biz.defaultCurrency || "ZAR"} tier={tier} onNew={() => { const mu = quotesThisMonth(quotes); if (TIERS[tier].quotesPerMonth !== Infinity && mu >= TIERS[tier].quotesPerMonth) { notify("Monthly limit reached.", "error"); return; } setAq(newQuote(biz.defaultCurrency)); setPage("editor"); }} onEdit={q => { setAq({ ...q }); setPage("editor"); }} search={search} setSearch={setSearch} />}
        {page === "editor" && aq && <Editor initial={aq} clients={clients} catalog={catalog} allQuotes={quotes} tier={tier} biz={biz} onSave={saveQ} onDelete={delQ} onBack={() => go("dashboard")} notify={notify} />}
        ...
        {page === "settings" && <Settings biz={biz} tier={tier} setTier={setTier} onSave={setBiz} notify={notify} />}
      </main>
