...
Email: Gmail SMTP (configurable SMTP provider)
...
|---|---:|---:|---:|---:|---|---|
| Free | · | 50 | 3 | 10 | No | No |
| Growth | R1,099/month | 300 | 250 | 500 | Yes | Yes |
| Business | R1,699/month | Unlimited | Unlimited | Unlimited | Yes | Yes |
...
Production email verification and password resets are delivered through the configured SMTP provider (Gmail by default) when SMTP_USER, SMTP_PASSWORD and EMAIL_FROM are configured. The app also queues quote, invoice, reminder, and direct client emails through the same SMTP transport, with monthly client-email allowances enforced by plan. System authentication emails are not charged against the customer's in-app client-email allowance. citeturn967677view0

Password reset links are generated by Better Auth and delivered through the same configured sender. For Gmail, use a Google App Password rather than your normal Gmail password.
